    ...
    function updateUI() {
        browserAPI.runtime.sendMessage({ action: 'getStats' }, (stats) => {
            ...
            if (domainList && stats.blockedByDomain) {
                ...
                if (domains.length === 0) {
                    domainList.innerHTML = '<div style="text-align:center;color:#666;padding:20px;font-size:12px;">No surveillance blocked yet</div>';
                } else {
                    domainList.innerHTML = domains.map(([domain, count]) => {
                        ...
                        return `<div class="domain-item"><span class="domain-name">${company}</span><span class="domain-count">${count}</span></div>`;
                    }).join('');
                }
            }
        });
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            browserAPI.runtime.sendMessage({ action: 'getStats' }, (stats) => {
                ...
                browserAPI.runtime.sendMessage({ action: 'getDiagnostics' }, (diag) => {
                    const data = { timestamp: new Date().toISOString(), version: '1.5.0', stats, diagnostics: diag };
    ...
    if (pauseBtn) {
        pauseBtn.addEventListener('click', () => {
            browserAPI.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                ...
                browserAPI.runtime.sendMessage({ action: 'pauseSite', domain, duration: 600000 }, (response) => {
                    if (response && response.success) alert(`Paused on ${domain} for 10 minutes`);
                });
            });
        });
    }
