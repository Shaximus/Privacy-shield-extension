...
//
// Safari/iOS Note:
// This script runs in MAIN world (supported since Safari 16.4).
// Fetch/XHR interception works identically to Chrome.
// The only change is using browserAPI wrapper for runtime.sendMessage.
...
(function() {
  ...
  function showSwapAlert(requested, returned) {
    ...
    // Log to extension — wrapped in try/catch since MAIN world scripts
    // may not always have access to the extension runtime on Safari
    try {
      ...
      // MAIN world may not have runtime access on some Safari versions
      console.warn('[AI Privacy Shield] Could not send swap report to extension:', e.message);
    }
  }
